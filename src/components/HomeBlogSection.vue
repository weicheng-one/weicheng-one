...
<template>
  <div class="bg-white pt-20 pb-24 sm:pb-32">
      ...
      >
        ...
        >
          <div class="relative w-full">
            <svg
              v-if="post.imageUrl == '' || !post.imageUrl"
              class="absolute w-12 h-12 inset-1/2 -translate-x-1/2 -translate-y-1/2"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
          ...
          <div class="max-w-xl">
            <div class="mt-8 flex items-center gap-x-4 text-xs">
              <time
                :datetime="(post.date.seconds * 1000).toString()"
                class="text-gray-500"
                >{{
                  useDateFormat(post.date.seconds * 1000, "MMM DD, YYYY").value
                }}</time
